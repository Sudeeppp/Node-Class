const { error } = require("console");
const express = require("express");
const fs = require("fs");
const cors = require("cors");

const app = express();

//middleware
app.use(express.json()); //json parser middleware

app.use(cors()); //allows every other origin

// app.use(cors({origin:"http://localhost:5173"})) // specific origin

//api
app.get("/", (req, res) => {
    res.send("Server at working");
});

// get all tasks

app.get("/alltasks", (req, res) => {
    const tasks = JSON.parse(fs.readFileSync("./Todo.json", "utf-8"));
    res.status(200).send(tasks);
});

//add  tasks

app.post("/add", (req, res) => {
    const { id, task } = req.body;
    if (!id || !task) {
        res.status(400).send({ error: "id and task is required" });
    }
    else {
        const allTasks = JSON.parse(fs.readFileSync("./Todo.json", "utf-8"));
        const isduplicate = allTasks.find(ele => ele.id == id);
        if (isduplicate) {
            res.status(400).send({ error: "id already exist" });
        }
        else {
            allTasks.push({ id, task });
            fs.writeFileSync("./Todo.json", JSON.stringify(allTasks));
            res.status(200).send({ msg: "task added successfully" });
        }
    }
})

//delete task

app.delete("/delete", (req, res) => {
    try {
        const { id } = req.body;
        if (!id) {
            res.status(400).send({ error: "send id to delete task" });
        }
        else {
            const allTasks = JSON.parse(fs.readFileSync("./Todo.json", "utf-8"));
            const isId = allTasks.find(ele => ele.id == id);
            if (!isId) {
                res.status(200).send({ msg: " id is not present and task deleted successfully" });
            }
            else {
                const updatedTasks = allTasks.filter(ele => ele.id != id);
                fs.writeFileSync("./Todo.json", JSON.stringify(updatedTasks));
                res.status(200).send({ msg: "task deleted from server " });
            }
        }
    }
    catch (err) {
        res.status(500).send(error);
    }

})

// edit task

app.put("/update", (req, res) => {
    try {
        const { id, newTask } = req.body;
        if (!id || !newTask) {
            res.status(400).send({ error: "Send all required fields" });
        }
        else {
            const allTasks = JSON.parse(fs.readFileSync("./Todo.json", "utf-8"));
            const isId = allTasks.find(ele => ele.id == id);
            if (!isId) {
                res.status(400).send({ error: "id not found" });
            }
            else {
                const updatedTasks = allTasks.map(ele => {
                    if (ele.id == id) {
                        ele.task = newTask;
                    }
                    return ele;
                });
                fs.writeFileSync("./Todo.json", JSON.stringify(updatedTasks));
                res.status(200).send({ msg: "task updated successfully" });
            }
        }
    }
    catch (err) {
        res.status(500).send(error);
    }
})


const port = 7000;
const hostname = "127.0.0.7";

app.listen(port, hostname, () => {
    console.log(`server is started at http://${hostname}:${port}`);
});